...
%%% Evaluate correlation between maximum temperature on a given day in Calgary and the maximum temperature on the same day in Winnipeg.
figure(6);clf;
% Find rows that aren't NaNs in both vectors
rows_to_use2 = find(~isnan(Calgary.data(:,4)) & ~isnan(Winnipeg.data(:,4)));
plot(Calgary.data(rows_to_use2,4),Winnipeg.data(rows_to_use2,4),'.','Color',[0.7,0.7,0.7]);
title('Daily maximum temperature comparison, Calgary-Winnipeg','FontSize',10);
xlabel('T_{max} Calgary (°C)','FontSize',14);
ylabel('T_{max} Winnipeg (°C)','FontSize',14);
set(gca,'FontSize',14);

% Use polyfit to fit a linear trend to the data. 
p2 = polyfit(Calgary.data(rows_to_use2,4),Winnipeg.data(rows_to_use2,4),1); % The '1' indicates a first-order model (i.e. line)
% Use polyval to create the linear estimates between -40 and 40
Wpg_Tmax_pred = polyval(p2,[-40:10:40]);
% Calculate correlation coefficient (R)
[R,p_value] = corrcoef(Calgary.data(rows_to_use2,4),Winnipeg.data(rows_to_use2,4));

% Add it to the figure:
figure(6);
hold on;
plot([-40:10:40],Wpg_Tmax_pred,'k--'); %plot the predicted values (i.e. trend line)
